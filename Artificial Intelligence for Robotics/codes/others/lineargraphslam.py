...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for i,item in enumerate(data):
        ...
        for landmark,dx,dy in sense:
            Ahelp = matrix([[1,-1],[-1,1]])
            bxhelp = matrix([[-dx],[dx]])
            byhelp = matrix([[-dy],[dy]])
            Ahelp = Ahelp.expand(nA,nA,[i,N+landmark],[i,N+landmark])
            bxhelp = bxhelp.expand(nA,1,[i,N+landmark],[0])
            byhelp = byhelp.expand(nA,1,[i,N+landmark],[0])
            A += Ahelp
            bx += bxhelp
            by += byhelp
        ...
        bxhelp = matrix([[-dx],[dx]])
        byhelp = matrix([[-dy],[dy]])
        Ahelp = Ahelp.expand(nA,nA,[i,i+1],[i,i+1])
        bxhelp = bxhelp.expand(nA,1,[i,i+1],[0])
        byhelp = byhelp.expand(nA,1,[i,i+1],[0])
        A += Ahelp
        bx += bxhelp
        by += byhelp
    ...
    v = []
    for i in xrange(nA):
        v.append([x.value[i][0]])
        v.append([y.value[i][0]])
